...
<head>
    <style>
        .editboxtitle .num{width:16px!important;height:16px!important;}
        .fa-title{width:14px!important;height:14px!important;margin-top:-5px;}
        .editboxtitle{padding:5px 10px!important;}
    </style>
</head>
<body>
...
if(__areas){
    for(var i in __areas){
        var __area_container = $('<div style="cursor:pointer;position:relative;"></div>');
        var __area_urls = new Array();
//        debugger;
        __areaName = __areas[i].areaName;
        __componentModels = __areas[i].componentModels;
        if(__componentModels.length > 0){
            __areaName_div = $('<div class="editboxtitle"><i class="num"><i class="fa fa-cube fa-title"></i></i>' + __areaName + '</div>');
            __area_container.append(__areaName_div);
            for(var x = 0 ; x < __componentModels.length ; x++){
                var __componentModel = __componentModels[x];
                __area_urls.push(_PATH+'/' + __componentModel.component + '?processInstID=' + __componentModel.processInstID + '&fromPage=<%=__fromPage%>&currentProcessInstID=' + _winParams.processInstID + '&taskInstID=' + _winParams.taskInstID + '&type=' + __componentModel.type);
            }
        } else {
            __areaName_div = $('<div class="editboxtitle1">' + __areaName + '</div>');
        }
        __bindAction2AreaName(__areaName_div , __area_urls);
        $('#__areaName_list').append(__area_container);
    }
} else {
...
//        __areaName_div = $('<div class="big-title-metar"><a href="'+_PATH+'/base/frame/frame.jsp?__area=' + encodeURIComponent(JsonObjectToString(__buildJSON)) + '" target="_blank">' + __areaName + '</a></div>');
        __areaName_div = $('<div class="editboxtitle"><i class="num"><i class="fa fa-comment-o"></i></i><a href="'+_PATH+'/base/frame/frame.jsp?__area=' + encodeURIComponent(JsonObjectToString(__buildJSON)) + '" target="_blank">' + __areaName + '</a></div>');
    } else {
//        __areaName_div = $('<div class="big-title-metar">' + __areaName + '</div>');
        __areaName_div = $('<div class="editboxtitle1"><i class="num"><i class="fa fa-comment-o"></i></i>' + __areaName + '</div>');
    }
...
//__areaName_div.empty().append($('<div>' + __areaName + '</div>'));
    __areaName_div.empty().append($('<div><i class="num"><i class="fa fa-cube fa-title"></i></i>' + __areaName + '</div>'));
...
//            }
            if(!document.getElementById('__' + __link_file + '_container') || !__link_dialog_show){
                __link_dialog = $('<div id="__' + __link_file + '_container" class="__link_dialog_container modal" style="display:none;"></div>');
...
//                    maxmin: true,
//                    area: ['700px', '400px'],
//                    content: __link_dialog  //这里content是一个DOM，注意：最好该元素要存放在body最外层，否则可能被其它的相对元素所影响
////                    btn: ['提交', '关闭'],
...
function __bindAction2AreaName(areaName , URLS){
//    alert(Math.uuid());
    for(var i = 0 ; i < URLS.length ; i++){
        var url = URLS[i];
        var componentDiv = $('<div class="area-div"></div>');
        areaName.parent().append(componentDiv);
        if(url.indexOf('FlowFileHome.do') > 0||url.indexOf('reviewAndTest.do') > 0){
            var b = Math.random();
            var iframeID = (b + new Date().getTime()).toString(16).replace(".", "");
            if(url.indexOf('type=edit') > -1){
                iframeID = 'edit-' + iframeID;
                areaName.parent().css({position : 'absolute' , width : '100%' , top : '40px' , bottom : 0});
                componentDiv.css({position : 'absolute' , bottom : 0 , top : '30px' , width : '100%'});
                $('html , body').css({height : '100%'});
            } else {
                iframeID = 'show-' + iframeID;
            }
            if(url.indexOf('?') > -1){
            ...
            componentDiv.append('<iframe id="'+iframeID+'" style="width:100%;height:100%;border:none;"  src="'+url+'"></iframe>');
//            alert(iframeID);
//            setTimeout(function(){
//                __setIFrameHeight(iframeID , );
//            } , 1000);
        } else {
            componentDiv.load(url);
        }
    }
    areaName.click(function(){
        var areaDiv = areaName.parent().find('.area-div');
        if(areaDiv.length > 0){
            areaDiv.slideToggle();
        }
    });
...
* @private
 * */
function __setIFrameHeight(iframeID , height){
//    alert(iframeID +','+ height);
//    alert($('#' + iframeID).length);
    if(iframeID.indexOf('show') > -1){
        $('#' + iframeID).height(height + 130);
    }
}

    function closeWindow(){
        if (window.opener.refresh_out_fetter_data) {
            window.opener.refresh_out_fetter_data();
        }
        setTimeout(function () {
            window.opener = null;
            window.open('', '_self');
