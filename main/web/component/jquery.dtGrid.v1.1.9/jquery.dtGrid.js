			...
			var dtGridObject = {
				...
				load : function(__callback){
					...
					if(dtGridReflectionObj.init.toolsIsInit==false){
						...
						if(!dtGridReflectionObj.option.ajaxLoad){
							dtGridReflectionObj.hideProcessBar(function(){
                                ...
                                try {
                                    if (__dtgrid_load_callback) {
                                        __dtgrid_load_callback();
                                    }
                                } catch(e){}
                                var __afterRender = dtGridReflectionObj.option.afterRender;
                                if(__afterRender){
                                    __afterRender();
                                }
                                if(dtGridReflectionObj.option.headerFloat){
                                    var gridContainer = $('#' + dtGridReflectionObj.option.gridContainer);
                                    var tables = gridContainer.find('table');
                                    var gridContainerHeight = gridContainer.height();
                                    var bodyContainer = $('#dt_grid_'+dtGridReflectionObj.option.id+'-tbody-container');
                                    var header = $(tables[0]);
                                    var newTbodyHeight = gridContainerHeight - header.height() - 20;
                                    bodyContainer.css({height:newTbodyHeight + 'px' , overflowY:'auto'});
                                }
                                //END
							});
							return;
					...
					if(!dtGridReflectionObj.option.ajaxLoad||dtGridReflectionObj.option.loadAll){
                        ...
                        //add by wangzwty@inspur.com
                        //BEGIN
                        pager.sort = dtGridReflectionObj.sortParameter?dtGridReflectionObj.sortParameter.columnId+','+dtGridReflectionObj.sortParameter.sortType:'';
                        //END
				...
				constructGrid : function(){
                    ...
                    //add by wangzwty@inspur.com
                    //BEGIN
                    gridContent += '<div id="dt_grid_'+dtGridReflectionObj.option.id+'-header-container" style="overflow-x: hidden;">';
                    //END

					gridContent += '<table class="dt-grid '+dtGridReflectionObj.option.tableClass+'" id="dt_grid_'+dtGridReflectionObj.option.id+'-header" style="margin-bottom:0;'+dtGridReflectionObj.option.tableStyle+'">';
					if(dtGridReflectionObj.option.showHeader!=false){
						var columns = dtGridReflectionObj.option.columns;

                        //add by wangzwty@inspur.com
                        //BEGIN
                        var titlespan = dtGridReflectionObj.option.titlespan;
                        if(titlespan){
                            gridContent += '<colgroup>';
                            for(var i=0; i<columns.length; i++) {
                                gridContent += '<col style="' + columns[i].headerStyle + '"></col>';
                            }
                            gridContent += '<colgroup>';
                        }
                        //END
                        ...
                        if(titlespan){
                            ...
                            for(var i = 0 ; i < titlespan.length; i++){
                                var titleTH = titlespan[i];
                                gridContent += '<th class="dt-grid-header" colspan="'+titleTH.colspan+'" style="'+titleTH.headerStyle+'">'+titleTH.title+'</th>';
                            }
                            gridContent += '	</tr>';
                        }
                        //END
						...
						if(dtGridReflectionObj.option.check){
							gridContent += '	<th class="check-column" style="'+dtGridReflectionObj.option.checkColumnStyle+'"><input type="checkbox" id="dt_grid_'+dtGridReflectionObj.option.id+'_check" value="check"></th>';
						}
						for(var i=0; i<columns.length; i++){
                        ...
                        //add by wangzwty@inspur.com
                        //BEGIN
						gridContent += '</table></div>';
                        //END
					}

                    //add by wangzwty@inspur.com
                    //BEGIN
					gridContent += '<div id="dt_grid_'+dtGridReflectionObj.option.id+'-tbody-container"><table class="dt-grid '+dtGridReflectionObj.option.tableClass+'" id="dt_grid_'+dtGridReflectionObj.option.id+'" style="margin-bottom:0;'+dtGridReflectionObj.option.tableStyle+'">';
                    //END
                    //构建表格
                    gridContent += '	<tbody>';
					if(dtGridReflectionObj.exhibitDatas!=null){
						...
						for(var i=0; i<dtGridReflectionObj.exhibitDatas.length; i++){
							...
							if(dtGridReflectionObj.option.check){
								gridContent += '	<td class="check-column text-center" style="'+dtGridReflectionObj.option.checkColumnStyle+'"><input type="checkbox" dataNo="'+i+'" id="dt_grid_'+dtGridReflectionObj.option.id+'_check_'+i+'" class="dt-grid-check" value="'+i+'"></td>';
							}
							var columns = dtGridReflectionObj.option.columns;
							...
							for(var j=0; j<columns.length; j++){
								...
								gridContent += '		<p dataNo="'+i+'" columnNo="'+j+'" class="dt-grid-cell '+dtGridReflectionObj.getExtraColumnClassForHide(columns[j])+'" style="'+columns[j].columnStyle+'">';
                    ...
                    //BEGIN
					gridContent += '</div>';
                    $('#dt_grid_'+dtGridReflectionObj.option.id + '-header').remove();
                    $('#dt_grid_'+dtGridReflectionObj.option.id + '-header-container').remove();
                    $('#dt_grid_'+dtGridReflectionObj.option.id + '-tbody-container').remove();
                    //END
                    ...
                    //add by wangzwty@inspur.com
                    //BEGIN
                    //绑定tbody左右滚动方法，触发header左右滚动
                    var __setScroll = null;
                    $('#dt_grid_'+dtGridReflectionObj.option.id+'-tbody-container').scroll(function(){
                        if(__setScroll == null){
                            setTimeout(function(){
                                $('#dt_grid_'+dtGridReflectionObj.option.id+'-header-container').scrollLeft($('#dt_grid_'+dtGridReflectionObj.option.id+'-tbody-container').scrollLeft());
                                __setScroll = null;
                            } , 100)
                        }
					...
					//绑定排序方法
                    //alter by wangzwty@inspur.com
                    //old code : $('#dt_grid_'+gridId+' .can-sort').click(function(){
                    //BEGIN
					$('#dt_grid_'+gridId+'-header .can-sort').click(function(){
                    //END
						//获取列编号
						...
						//重新加载数据
                        //alter by wangzwty@inspur.com
                        //old code : dtGridReflectionObj.reload();
                        //原有代码reload只是将页面已加载的数据进行排序，修改成load后，会重新向后台请求新数据，并带有排序参数
                        //BEGIN
				...
				toolbar : function(type){
					...
					var toolbars = {
						...
						add : '<li title="'+$.fn.DtGrid.lang[dtGridReflectionObj.option.lang].toolbar.addTitle+'"><a href="javascript:void(0);" id="addGrid_replaceId">'+$.fn.DtGrid.lang[dtGridReflectionObj.option.lang].toolbar.add+'</a></li>'
					};
				...
				constructGridToolBar : function(){
					...
					for(var i=0; i<tools.split('|').length; i++){
                        ...
                        //新增
                        if(tool=='add'){
                            var content = $.fn.DtGrid.tools.replaceAll(dtGridReflectionObj.toolbar('add'), 'replaceId', dtGridReflectionObj.option.id);
                            $('#'+dtGridReflectionObj.option.toolbarContainer+' .dt-grid-tools').append(content);
                            //绑定方法
                            $('#addGrid_'+dtGridReflectionObj.option.id).click(function(){
                                if(dtGridReflectionObj.option.newRecordFunction){
                                    dtGridReflectionObj.option.newRecordFunction();
                                } else {
                                    alert('未配置属性：newRecordFunction');
                                }
                            });
                        }
					}
				...
				faseQuery : function(){
					...
					for(var j=0; j<dtGridReflectionObj.option.columns.length; j++){
						...
						if(column.fastQuery==true){
                            if(column.fqhide==true){
                                content += '<input type="hidden" class="form-control" id="'+column.fastQueryType+'_'+column.id+'" name="'+column.fastQueryType+'_'+column.id+'" placeholder="'+$.fn.DtGrid.lang[dtGridReflectionObj.option.lang].fastQuery.input+column.title+'" />';
                                continue;
                            }
							...
							if(column.fastQueryType=='range'){
								...
								if(column.type=='date'){
									...
									content += '</div>';
                                        ...
                                        content += '<input onclick="'+column.click+'()" type="text" class="form-control" id="'+column.fastQueryType+'_'+column.id+'" name="'+column.fastQueryType+'_'+column.id+'" placeholder="'+$.fn.DtGrid.lang[dtGridReflectionObj.option.lang].fastQuery.input+column.title+'" />';
...
//					});
				 ...
				 */
				//导出
				exportFile : function(exportType,isdirect){
					//映射参数
					var dtGridReflectionObj = this;
					//如果已经初始化，则显示导出选项
					...
					content += '				<div class="form-export modal-body form-horizontal form-export" style="display:none;">';
					content += '					<div class="form-group">';
					...
					content += '								<label><input type="radio" name="dt_grid_export_export_all_data_'+exportType+'_'+dtGridReflectionObj.option.id+'" value="0" /> '+$.fn.DtGrid.lang[dtGridReflectionObj.option.lang]['export'][exportType].exportType.now+'</label>';
					content += '								<label><input type="radio" name="dt_grid_export_export_all_data_'+exportType+'_'+dtGridReflectionObj.option.id+'" value="1" checked="checked"/> '+$.fn.DtGrid.lang[dtGridReflectionObj.option.lang]['export'][exportType].exportType.all+'</label>';
					content += '							</div>';
					content += '						</div>';
					content += '					</div>';
					content += '				</div>';
					...
					for(var i=0; i<dtGridReflectionObj.option.columns.length; i++){
						...
						if(column['exportNoCheck']==true){
							content += '					<tr id="dt_grid_export_tr_'+exportType+'_'+dtGridReflectionObj.option.id+'_'+i+'">';
							content += '						<td><input type="checkbox" id="dt_grid_export_check_'+exportType+'_'+dtGridReflectionObj.option.id+'_'+i+'"  value="'+i+'" /></td>';
							content += '						<td>'+column.title+'</td>';
							content += '						<td>';
                ...
                /**
                 * 合并单元格方法
                 * @param rowNo 合并单元格的行号
                 * @param colNo 合并单元格的列号
                 * @param rowSpan   合并行数
                 * @param colSpan   合并列数
                 */
                spanTD : function(rowNo , colNo , rowSpan , colSpan){
                    var dtGridReflectionObj = this;
                    var trs = $('#dt_grid_' + dtGridReflectionObj.option.id).find('tbody').find('tr');
                    var tr = trs[rowNo*2];
                    var tds = $(tr).find('td');
                    var td = tds[colNo];
                            ...
                            for(var rowIndex = 0; rowIndex  < rowSpan ; rowIndex++){
                                for(var colIndex = 0; colIndex  < colSpan ; colIndex++){
                                    if(rowIndex == 0){
                                        ...
                                        var td1 = tds[colNo + colIndex];
                                        $(td1).remove();
                                    } else {
                                        var tr1 = trs[rowNo*2 + rowIndex*2];
		...
		tools : {
			...
			getWindowStart : function(id, title){
				...
				content += '		<div class="modal-content" style="height:500px;overflow: auto;">';
